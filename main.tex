...
To measure how well our system works, we use the Confusion Matrix. This helps us see if the AI is making the right guesses. The main formulas we use are:
...
\caption{Full workflow process of the FedSearch-NLP system.}
...
\textbf{Configuration B (Server-Only update)}: This uses very little data but doesn't learn as well.
\begin{table}[h]
\centering
...
\textbf{Configuration C (Full LoRA Update)}: This is the best balance for most users. It uses much less data than Config A but still learns very well.
\begin{table}[h]
\centering
...
\begin{itemize}
    \item \textbf{Configuration A}: Best quality but too much data for most users.
    \item \textbf{Configuration B}: Very fast but the AI does not learn enough.
    \item \textbf{Configuration C}: The best choice for most companies. It is fast and still gives very good results.
    \item \textbf{Configuration D}: The most powerful option for researchers who need absolute accuracy.
\end{itemize}
...
In this paper, we presented FedSearch-NLP. This is a system that allows organisations to train AI together while keeping their document collections private. We solved several major problems with federated learning for RAG systems, including the high cost of data transfer and the difficulty of reading complex business documents with tables and images.

Our results with Apple and Microsoft documents show that our system works. We achieved up to 99.8\% accuracy, and our practical "Config C" setup offers a great balance of speed and quality. This makes FedSearch-NLP a powerful tool for fields like finance and medicine where privacy is essential.
...
We would like to thank the open-source community for the tools used in this project, including Hugging Face, Sentence-Transformers, and PyTorch.
